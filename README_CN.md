...
ComfyUI 自定义节点工具箱，主要围绕多图参考、SAM3/SAM3.1 人物抠图拼接、SCAIL-2 遮罩处理、布尔值传递，以及工作流分组切换进行扩展。
...
| `InvertBooleanFeiHou` | 反转布尔值 | v2.3 |
...
- 新增 `InvertBooleanFeiHou（反转布尔值）` 节点，只有一个布尔输入和一个布尔输出。
- 兼容 `PrimitiveBoolean` 输出链路，可将 `true` 反转为 `false`，将 `false` 反转为 `true`。
- 新增节点标题语言适配，中文界面显示 `反转布尔值`，其他语言显示 `Invert Boolean`，标题栏不会同时显示中英文。
- 新增 `locales/en` 和 `locales/zh` 节点本地化定义。
